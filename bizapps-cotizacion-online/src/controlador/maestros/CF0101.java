...
public class CF0101 extends CGenerico {
	...
	public void inicializar() throws IOException {
		...
		settearTituo(divVF0101, idArbol);
		mostrarCatalogo();
		// Ajustar a 3 el long
		cargarBuscadores();
		...
		botonera = new Botonera() {
			...
			public void seleccionar() {
				if (validarSeleccion()) {
					if (catalogo.obtenerSeleccionados().size() == 1) {
						...
						if (a != null) {
							...
							if (a.length() <= 3) {
								...
								if (a.length() > 3 && a.length() <= 7) {
									txtDUNS2F0101.setValue(f01.getAbduns()
											.trim().substring(3, a.length()));
								} else {
									txtDUNS2F0101.setValue(f01.getAbduns()
											.trim().substring(3, 7));
									txtDUNS3F0101.setValue(f01.getAbduns()
											.trim().substring(7));
								}
							}
						}
						...
						buscadorTAXC.settearCampo(f01.getAbtaxc());
			...
			public void guardar() {
				...
				if (guardar) {
					F0101 f01 = new F0101();
					if (clave == 0)
						clave = nextNumber("01", "DIR", "Direcciones");
					f01.setAban8(clave);
					...
					f01.setAbtaxc(buscadorTAXC.obtenerCaja());
				...
				if (gpxDatosF0101.isOpen()) {
					...
					if (validarSeleccion()) {
						...
						for (int i = 0; i < eliminarLista.size(); i++) {
							...
							List<Usuario> objetoF0006 = servicioUsuario
	...
	private void cargarBuscadores() {
		...
		String ancho3P1 = "25%";
		String ancho4P1 = "23%";
		buscadorAT1 = crearCampoUDC(divBuscadorAT1, "Clasificacion:", true,
